...
Promise.all(promises).then((value)=>{
    ...
    let root=d3.hierarchy(movie).sum((d)=>d.value).sort((a,b)=>b.height-a.height || a.value - b.value);
    tree(root);
    ...
    legend.selectAll('rect').data(cate).enter().append('rect').attr('x',(d,i)=>(i*80)).attr('y',10).attr('height',30).attr('width',40).attr('class','legend-item').attr('fill',(d)=>color(d)).append('title').text((d,i)=>d+', '+col[i])
    legend.selectAll('text').data(cate).enter().append('text').attr('x',(d,i)=>(i*80)).attr('y',55).text((d)=>d)
})
